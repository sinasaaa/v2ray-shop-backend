// ===== FINAL & COMPLETE CODE (Section: src/bot.ts) - NO DELETIONS =====

// ===== IMPORTS & DEPENDENCIES =====
import { Telegraf, Context, Markup, session } from 'telegraf';
import { message } from 'telegraf/filters';
import logger from './utils/logger';
import { PrismaClient, UserRole } from '@prisma/client'; // Import UserRole as well
...
// Add handlers for other admin buttons here (as you build them)
...
// ... other admin handlers for 'آمار فروش', 'لیست کاربران'
    ...
    if (scene.startsWith('add_plan_')) {
        switch (scene) {
            ...
            case 'add_plan_datalimit':
                ...
                await prisma.plan.create({ data: { ...ctx.session.planData, dataLimit: dataLimitBytes } });
